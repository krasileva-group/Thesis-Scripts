...
readCount1 <- read.table("../ReadCountAdjustedTreatTime-1hr-B1-B19-P21-B20.csv", header=TRUE, sep=",", row.names=1)
...
topBvsP1<- topTags(BvsP, n=nrow(keep1), adjust.method = "BH", sort.by = "PValue", p.value = 0.05)
topBvsH1<- topTags(BvsH, n=nrow(keep1), adjust.method = "BH", sort.by = "PValue", p.value = 0.05)
topHvsP1<- topTags(HvsP, n=nrow(keep1), adjust.method = "BH", sort.by = "PValue", p.value = 0.05)


topBvsP1np<- topTags(BvsP, n=nrow(keep1), adjust.method = "BH", sort.by = "PValue")
topBvsH1np<- topTags(BvsH, n=nrow(keep1), adjust.method = "BH", sort.by = "PValue")
topHvsP1np<- topTags(HvsP, n=nrow(keep1), adjust.method = "BH", sort.by = "PValue")
...
write.table(topBvsP1, file="topTags-BvsP-1hr-Batch-glmLRT.csv", sep=',', quote=FALSE)
write.table(topBvsH1, file="topTags-BvsH-1hr-Batch-glmLRT.csv", sep=',', quote=FALSE)
write.table(topHvsP1, file="topTags-HvsP-1hr-Batch-glmLRT.csv", sep=',', quote=FALSE)
...
write.table(CombiMarkersBP1, file="topTags-BvsP-1hr-Batch-glmQLF-Marker.csv", sep=',', quote=FALSE)
write.table(CombiMarkersHP1, file="topTags-BvsH-1hr-Batch-glmQLF-Marker.csv", sep=',', quote=FALSE)
write.table(CombiMarkersBH1, file="topTags-HvsP-1hr-Batch-glmQLF-Marker.csv", sep=',', quote=FALSE)
